...
function fullBodyProfilePrompt() {
  return [
    ...
    'ABSOLUTE FACE RULE: Preserve the face from the uploaded photo as the identity anchor. Do not redesign, beautify, smooth, slim, age, de-age, re-face, or reinterpret the face in any way. Preserve the same face shape, eye shape, eyelids, eye spacing, eyebrows, nose, lips, jawline, chin, ears, hairline, hairstyle, skin tone, skin texture, and any visible marks or moles. The generated face must be instantly recognizable as the same person from the uploaded image.',
    'IMPORTANT IDENTITY PRIORITY: Do not invent a new perfectly front-facing version of the face. If the uploaded face is tilted, angled, three-quarter, or slightly turned, keep the same facial structure, eye shape, eyelids, expression, smile shape, and natural head character from the uploaded photo. The body should become a clean catalog standing pose, but the head and face may retain a slight natural angle when needed to preserve identity. Exact identity and expression preservation are more important than perfect pose symmetry.',
    'BODY & POSE: Exactly one person, complete full body visible from top of head to soles of feet. Straight, relaxed standing pose, body mostly squared to the camera, arms relaxed at the sides, both hands and all fingers visible, feet slightly apart, weight evenly balanced. If the reference is a selfie, cropped portrait, or half-body photo, infer only the missing body below the visible region, consistent with the person\'s visible build, apparent age, and skin tone. Do not alter the face or expression to satisfy the body pose. No cropping at the head, shoulders, arms, hands, waist, hips, knees, ankles, or feet.',
    ...
    'DO NOT: Do not modify the face, eyes, eyelids, eyebrows, expression, smile, hairstyle, skin tone, or identity. Do not add logos, text, watermarks, accessories, jewelry, hats, sunglasses, bags, or props. No extra people, mirrors, reflections, or duplicated limbs. No stylization, cartoon, illustration, or beauty filters. Output must be photorealistic, modest, non-sexualized, and suitable as an ecommerce body reference.'
  ].join(' ');
}
